	...
	public static void main(String args[]) throws IOException,FileNotFoundException{
		...
		for(int i =1;i<201;i++)
		{
			...
			Sol="As the measure of base is not given let us assume base to be $"+var1[p]+"$ cm. <br>" +
								...
								"$\\therefore "+var1[p]+"^2+"+((div/nGcd)==1?"("+var1[p]+"+"+(diff/div)+")":"\\Bigl(\\dfrac {"+(div/nGcd)+""+var1[p]+"+"+(diff/nGcd)+"}{"+(div/nGcd)+"}")+"\\Bigl)^2="+((div/nGcd1)==1?"("+(Chr3[nim]/div)+")":"\\Bigl(\\dfrac {"+(Chr3[nim]/nGcd1)+"}{"+(div/nGcd1)+"}")+"\\Bigl)^2$ . . . .  by Pythagoras theorem <br>" +
								...
								"$\\therefore "+(cf1==1?"":cf1)+""+var1[p]+"\\Bigl("+(term2/cf1==1?"":""+term2/cf1+"")+""+var1[p]+"+"+cf4/cf1+"\\Bigl)-"+(cf2==1?"":cf2)+"\\Bigl("+(cf5/cf2==1?"":""+cf5/cf2+"")+""+var1[p]+"+"+term4/cf2+"\\Bigl)=0$ <br>" +
								"$\\therefore \\Bigl("+(term2/cf1==1?"":""+term2/cf1+"")+""+var1[p]+"+"+cf4/cf1+"\\Bigl)\\Bigl("+(cf1==1?"":cf1)+""+var1[p]+"-"+(cf2==1?"":cf2)+"\\Bigl)=0$ <br>" +
								...
								"Other side $="+var1[p]+"+\\Bigl(\\dfrac {"+diff+"}{"+div+"}\\Bigl) = "+(ans1gcd==1?""+(cf1==1?""+(cf2/cf1)+"":"\\Bigl(\\dfrac {"+cf2+"}{"+cf1+"}\\Bigl)")+"":((cf1/ans1gcd)==1?"="+(cf2/cf1)+"":"\\dfrac {"+(cf2/ans1gcd)+"}{"+(cf1/ans1gcd)+"}"))+"+\\Bigl(\\dfrac {"+diff+"}{"+div+"}\\Bigl)=\\dfrac {"+Chr2[nim]+"}{"+div+"}"+(ans2gcd==1?"":((div/ans2gcd)==1?"="+(Chr2[nim]/div)+"":"=\\dfrac {"+(Chr2[nim]/ans2gcd)+"}{"+(div/ans2gcd)+"}"))+"$ cm <br>" +
			...
			Sol1="#पायाची किंमत दिलेली नसल्यामुळे आपण पाया $"+var1[p]+"$ सेंमी आहे असे मानू. <br>" +
								...
								"म्हणून या काटकोन त्रिकोणासाठी पायथागोरसच्या प्रमेया नुसार <br>" +
								"पाया $^2 +$ उंची $^2 = $ कर्ण $^2$ <br>" +
								"$\\therefore "+var1[p]+"^2+"+((div/nGcd)==1?"("+var1[p]+"+"+(diff/div)+")":"\\Bigl(\\dfrac {"+(div/nGcd)+""+var1[p]+"+"+(diff/nGcd)+"}{"+(div/nGcd)+"}")+"\\Bigl)^2="+((div/nGcd1)==1?"("+(Chr3[nim]/div)+")":"\\Bigl(\\dfrac {"+(Chr3[nim]/nGcd1)+"}{"+(div/nGcd1)+"}")+"\\Bigl)^2$ . . . .  पायथागोरस प्रमेया नुसार <br>" +
								...
								"$\\therefore "+(cf1==1?"":cf1)+""+var1[p]+"\\Bigl("+(term2/cf1==1?"":""+term2/cf1+"")+""+var1[p]+"+"+cf4/cf1+"\\Bigl)-"+(cf2==1?"":cf2)+"\\Bigl("+(cf5/cf2==1?"":""+cf5/cf2+"")+""+var1[p]+"+"+term4/cf2+"\\Bigl)=0$ <br>" +
								"$\\therefore \\Bigl("+(term2/cf1==1?"":""+term2/cf1+"")+""+var1[p]+"+"+cf4/cf1+"\\Bigl)\\Bigl("+(cf1==1?"":cf1)+""+var1[p]+"-"+(cf2==1?"":cf2)+"\\Bigl)=0$ <br>" +
								"$\\therefore "+(term2/cf1==1?"":""+term2/cf1+"")+""+var1[p]+"+"+(cf4/cf1)+"=0$ किंवा $"+(cf1==1?"":cf1)+""+var1[p]+"-"+(cf2==1?"":cf2)+"=0$ <br>" +
								"$\\therefore "+var1[p]+"="+(term2/cf1==1?"-"+((cf4)/cf1)+"":"-\\dfrac {"+(cf4)/cf1+"}{"+term2/cf1+"}")+"$ किंवा $"+var1[p]+"="+(cf1==1?""+(cf2/cf1)+"":"\\dfrac {"+cf2+"}{"+cf1+"}")+"$ <br>" +
								"परंतु $"+var1[p]+"="+(term2/cf1==1?"-"+((cf4)/cf1)+"":"-\\dfrac {"+(cf4)/cf1+"}{"+term2/cf1+"}")+"$ हे शक्य नाही (कारण ती त्रिकोणाची बाजू आहे) <br>" +
								"$\\therefore "+var1[p]+"="+(cf1==1?""+(cf2/cf1)+"":"\\dfrac {"+cf2+"}{"+cf1+"}")+""+(ans1gcd==1?"":((cf1/ans1gcd)==1?"="+(cf2/cf1)+"":"=\\dfrac {"+(cf2/ans1gcd)+"}{"+(cf1/ans1gcd)+"}"))+"$ सेंमी ही एक बाजू आहे <br>" +
								"दुसरी बाजू  $="+var1[p]+"+\\Bigl(\\dfrac {"+diff+"}{"+div+"}\\Bigl) = "+(ans1gcd==1?""+(cf1==1?""+(cf2/cf1)+"":"\\Bigl(\\dfrac {"+cf2+"}{"+cf1+"}\\Bigl)")+"":((cf1/ans1gcd)==1?"="+(cf2/cf1)+"":"\\dfrac {"+(cf2/ans1gcd)+"}{"+(cf1/ans1gcd)+"}"))+"+\\Bigl(\\dfrac {"+diff+"}{"+div+"}\\Bigl)=\\dfrac {"+Chr2[nim]+"}{"+div+"}"+(ans2gcd==1?"":((div/ans2gcd)==1?"="+(Chr2[nim]/div)+"":"=\\dfrac {"+(Chr2[nim]/ans2gcd)+"}{"+(div/ans2gcd)+"}"))+"$ सेंमी <br>" +
								"उरलेल्या दोन बाजू ह्या $"+(ans1gcd==1?""+(cf1==1?""+(cf2/cf1)+"":"\\dfrac {"+cf2+"}{"+cf1+"}")+"":((cf1/ans1gcd)==1?"="+(cf2/cf1)+"":"\\dfrac {"+(cf2/ans1gcd)+"}{"+(cf1/ans1gcd)+"}"))+"$ सेंमी आणि $"+(ans2gcd==1?"\\dfrac {"+Chr2[nim]+"}{"+div+"}":((div/ans2gcd)==1?""+(Chr2[nim]/div)+"":"\\dfrac {"+(Chr2[nim]/ans2gcd)+"}{"+(div/ans2gcd)+"}"))+"$ सेंमी आहेत हे उत्तर. <br>";
			...
			wrong_ans="$"+(cf2==1?""+cf1+"":(cf1%cf2==0?""+cf1/cf2+"":"\\dfrac {"+cf1+"}{"+cf2+"}"))+"$ cm and $"+(ans2gcd==1?"\\dfrac {"+Chr2[nim]+"}{"+div+"}":((div/ans2gcd)==1?""+(Chr2[nim]/div)+"":"\\dfrac {"+(Chr2[nim]/ans2gcd)+"}{"+(div/ans2gcd)+"}"))+"$ cm <br> #$"+(cf2==1?""+cf1+"":(cf1%cf2==0?""+cf1/cf2+"":"\\dfrac {"+cf1+"}{"+cf2+"}"))+"$ सेंमी आणि $"+(ans2gcd==1?"\\dfrac {"+Chr2[nim]+"}{"+div+"}":((div/ans2gcd)==1?""+(Chr2[nim]/div)+"":"\\dfrac {"+(Chr2[nim]/ans2gcd)+"}{"+(div/ans2gcd)+"}"))+"$ सेंमी";
			
			wrong_ans1="$"+(div==1?""+(Chr1[nim]+5)+"":((Chr1[nim]+5)%div==0?""+(Chr1[nim]+5)/div+"":"\\dfrac {"+(Chr1[nim]+5)+"}{"+div+"}"))+"$ cm and $"+(div==1?""+(Chr2[nim]+1)+"":((Chr2[nim]+1)%div==0?""+(Chr2[nim]+1)/div+"":"\\dfrac {"+(Chr2[nim]+1)+"}{"+div+"}"))+"$ cm <br> #$"+(div==1?""+(Chr1[nim]+5)+"":((Chr1[nim]+5)%div==0?""+(Chr1[nim]+5)/div+"":"\\dfrac {"+(Chr1[nim]+5)+"}{"+div+"}"))+"$ सेंमी आणि $"+(div==1?""+(Chr2[nim]+1)+"":((Chr2[nim]+1)%div==0?""+(Chr2[nim]+1)/div+"":"\\dfrac {"+(Chr2[nim]+1)+"}{"+div+"}"))+"$ सेंमी";
			
			wrong_ans2="$"+(div%Chr3[nim]==0 ?""+div/Chr3[nim]+"":"\\dfrac {"+(div)+"}{"+Chr3[nim]+"}")+"$ cm and $"+(div%diff==0 ?""+div/diff+"":"\\dfrac {"+(div)+"}{"+diff+"}")+"$ cm <br> # $"+(div%Chr3[nim]==0 ?""+div/Chr3[nim]+"":"\\dfrac {"+(div)+"}{"+Chr3[nim]+"}")+"$ सेंमी and $"+(div%diff==0 ?""+div/diff+"":"\\dfrac {"+(div)+"}{"+diff+"}")+"$ सेंमी";
