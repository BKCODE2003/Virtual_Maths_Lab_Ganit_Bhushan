...
public class VLab_0304070202_122_Assign30_Bhushan {
	...
	public static void main(String args[]) throws IOException,FileNotFoundException{
		...
		for(int i =1;i<201;i++)
		{
			...
			if(cf1==cf2){
			 wrong_ans="$"+(cf2==1?""+(cf1+cf2)+"":(cf1%cf2==0?""+(cf1+cf2)+"":"\\dfrac {"+cf1+"}{"+cf2+"}"))+"$ cm and $"+(ans2gcd==1?""+(div==1?""+Chr2[nim]/div+"":"\\dfrac {"+Chr2[nim]+"}{"+div+"}")+"":((div/ans2gcd)==1?""+(Chr2[nim]/div)+"":"\\dfrac {"+(Chr2[nim]/ans2gcd)+"}{"+(div/ans2gcd)+"}"))+"$ cm <br> #$"+(cf2==1?""+(cf1+cf2)+"":(cf1%cf2==0?""+(cf1+cf2)+"":"\\dfrac {"+cf1+"}{"+cf2+"}"))+"$ सेंमी आणि $"+(ans2gcd==1?""+(div==1?""+Chr2[nim]/div+"":"\\dfrac {"+Chr2[nim]+"}{"+div+"}")+"":((div/ans2gcd)==1?""+(Chr2[nim]/div)+"":"\\dfrac {"+(Chr2[nim]/ans2gcd)+"}{"+(div/ans2gcd)+"}"))+"$ सेंमी <br>";
